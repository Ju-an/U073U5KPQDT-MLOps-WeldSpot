...
def analyze_classes(files):
    class_metrics = {class_name: {'actuals': [], 'predictions': [], 'TP': 0, 'FP': 0, 'TN': 0, 'FN': 0} for class_name in CLASS_NAMES}
    ...
    for file in files:
        ...
        class_conf_pairs = class_conf_pairs[1:-4].split('_')  # Remove leading underscore and .jpg
    ...
    for class_name, metrics in class_metrics.items():
        if len(set(metrics['actuals'])) > 1:  # AUC is undefined if there is only one class in actuals
            class_aucs[class_name] = roc_auc_score(metrics['actuals'], metrics['predictions'])
        else:
